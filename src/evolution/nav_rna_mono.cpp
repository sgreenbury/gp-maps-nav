...
ABSL_FLAG(int, sources, 20, "Number of sources per target to test");
ABSL_FLAG(int, targets, 1, "Number of targets to test");
...
ABSL_FLAG(int, threshold, 2000000, "Computational threshold for size of u + v");
ABSL_FLAG(int, population_size, 100,
          "Population size for monomorphic evolution regime");
ABSL_FLAG(int, shape_level, 0,
          "Shape level for the RNA assembly. Default: 0 (dot-bracket).");
ABSL_FLAG(std::string, fitness_measure, "random",
          "Fitness assignment is \"random\" or \"hamming\"");
ABSL_FLAG(bool, verbose, false, "Print progress");
ABSL_FLAG(bool, load_genos, false,
          "Load genotypes four sources from specified --geno_fname");
ABSL_FLAG(bool, record_diffs_only, false,
          "Record only changes in phenotype to file to allow longer walks");
ABSL_FLAG(bool, debug, false, "Run in debug mode");
ABSL_FLAG(std::string, geno_fname, "cs_l20.txt", "Input genotype file path");
...
ABSL_FLAG(std::string, pheno_fname, "ps_l20.txt", "Input phenotype file path");
...
void WriteFlags(std::string configfile) {
  ...
  config << "population_size: " << absl::GetFlag(FLAGS_population_size)
         << std::endl;
  config << "shape_level: " << absl::GetFlag(FLAGS_shape_level) << std::endl;
  ...
  config << "load_genos: " << absl::GetFlag(FLAGS_load_genos) << std::endl;
  config << "record_diffs_only: " << absl::GetFlag(FLAGS_record_diffs_only)
         << std::endl;
...
int main(int argc, char *argv[]) {
  std::string message =
      "Evolutionary navigability for monmorphic regime in fRNA.";
  ...
  /// Make FW object
  FW_frna fw_frna(
      absl::GetFlag(FLAGS_base), absl::GetFlag(FLAGS_length),
      absl::GetFlag(FLAGS_nd_ref), absl::GetFlag(FLAGS_targets),
      absl::GetFlag(FLAGS_sources), absl::GetFlag(FLAGS_all),
      absl::GetFlag(FLAGS_neutral_mutations), absl::GetFlag(FLAGS_threshold),
      absl::GetFlag(FLAGS_population_size), absl::GetFlag(FLAGS_shape_level),
      absl::GetFlag(FLAGS_fitness_measure), absl::GetFlag(FLAGS_outpath),
      absl::GetFlag(FLAGS_file_name), absl::GetFlag(FLAGS_geno_fname),
      absl::GetFlag(FLAGS_load_genos), absl::GetFlag(FLAGS_pheno_fname),
      absl::GetFlag(FLAGS_record_diffs_only), absl::GetFlag(FLAGS_debug));

  // Perform DFS across sources and targets
  fw_frna.RunMono();
